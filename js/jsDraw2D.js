...
function jsPen(color, width) {
	...
	//color proprty of jsColor type
	this.width = "1px";
	//width property with 1px default value
...
function jsGraphics(canvasDivElement) {
	...
	var scale = 1;
	var coordinateSystem = "default";
	//Possible values "default" or "cartecian"
	var canvasDiv;
	...
	//Document will be used directly for drawing
	...
	//lmk
	this.ww = canvasDiv.clientWidth;
	this.hh = canvasDiv.clientHeight;
	//lmk
	...
	function showGrid(range, showRange, color) {
		if (showRange == null)
			showRange = true;
		//range is grid interval. The values will be shown if showRange is true.

		var x0, x1, y0, y1;
		var isLeft = false;
		//range display on left side of y-axis if true otherwise right side.
		var isUp = false;
		//range display above the x-axis if true otherwise below.
		gridDiv.innerHTML = "";
		...
		if (!range)
			range = Math.round(parseInt(gridDiv.style.width) / 10);
		//If range not specified, use grid with devided by 10 as range.
		else
			range = range * scale;
	...
	function fillArc(color, center, width, height, startAngle, swapAngle) {
		...
		function fillArcSegIn(isUpperHalf, valueOnly) {
			...
			if (isUpperHalf) {
				var draw1 = false;
				//upper half (in all comments upper & lower are in context of cartecian system)
				var draw3 = false;
				//upper half second
				divY = divY1;
