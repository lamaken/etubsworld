...
function drawPath(points, color) {
	...
	if (len == 2) {
		context.bezierCurveTo(points[0].x, points[0].y, points[0].x, points[0].y, points[1].x, points[1].y);
	}
	if (len == 3) {
	...
	if (len == 5) {
		context.bezierCurveTo(points[1].x, points[1].y, points[2].x, points[2].y, points[4].x, points[4].y);

	}
	if (len > 5) {
		for (var step = 1; step < len; step += 1) {
			context.lineTo(points[step].x, points[step].y);
			//context.bezierCurveTo(points[step].x, points[step].y, points[step+1].x, points[step+1].y, points[step+2].x, points[step+2].y);
		}
	}
	//drawtext context.
	context.stroke();
